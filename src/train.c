...
/* ---- multithreaded batch worker pool (fork-join per batch) ---- */

#define SJEV_MAX_THREADS 1024

struct sjev_worker_arg {
    const struct sjev_model *m;
    const struct dataset *ds;
    const size_t *indices;
    size_t start, end;          /* sub-range [start,end), read-only */
    struct sjev_grad *grad;     /* private to this worker */
    double loss_sum;            /* private to this worker */
    int rc;                     /* private to this worker */
    char err[256];              /* private to this worker */
};

struct sjev_thread_pool {
    int nthreads;
    struct sjev_grad *grads;    /* [nthreads] private gradient buffers */
};

static void grad_add_array(float *dst, const float *src, size_t n) {
    for (size_t i = 0; i < n; i++) dst[i] += src[i];
}

static void grad_add(const struct sjev_model *m, struct sjev_grad *dst, const struct sjev_grad *src) {
    ...
    grad_add_array(dst->embedding, src->embedding, emb_n);
    grad_add_array(dst->position, src->position, pos_n);
    grad_add_array(dst->ln_c_w, src->ln_c_w, norm_n);
    grad_add_array(dst->ln_c_b, src->ln_c_b, norm_n);
    grad_add_array(dst->ln_o_w, src->ln_o_w, norm_n);
    grad_add_array(dst->ln_o_b, src->ln_o_b, norm_n);
    grad_add_array(dst->Wq, src->Wq, qkv_n);
    grad_add_array(dst->Wk, src->Wk, qkv_n);
    grad_add_array(dst->Wv, src->Wv, qkv_n);
}

static void *sjev_worker_main(void *arg) {
    struct sjev_worker_arg *a = (struct sjev_worker_arg *)arg;
    /* Only our own grad/loss/rc/err are written here; model, dataset and
     * index array are read-only. forward_cache temporaries are malloc'd per
     * example inside forward_one/backward_one and never shared. */
    grad_zero(a->m, a->grad);
    a->loss_sum = 0.0;
    a->rc = 0;
    a->err[0] = '\0';
    for (size_t b = a->start; b < a->end; b++) {
        size_t idx = a->indices[b];
        struct forward_cache c;
        if (forward_one(a->m, &a->ds->examples[idx], &c, a->err, sizeof(a->err)) != 0) {
            a->rc = -1;
            return NULL;
        }
        a->loss_sum += c.loss;
        if (backward_one(a->m, &c, a->grad, a->err, sizeof(a->err)) != 0) {
            cache_free(&c);
            a->rc = -1;
            return NULL;
        }
...
struct sjev_thread_pool *sjev_pool_create(const struct sjev_model *m, int nthreads, char *err, size_t err_len) {
    if (!m || nthreads < 1 || nthreads > SJEV_MAX_THREADS) {
        if (err) snprintf(err, err_len, "thread count must be 1..%d", SJEV_MAX_THREADS);
        return NULL;
    }
    struct sjev_thread_pool *p = (struct sjev_thread_pool *)calloc(1, sizeof(*p));
    if (!p) {
        if (err) snprintf(err, err_len, "oom pool");
        return NULL;
    }
    p->nthreads = nthreads;
    p->grads = (struct sjev_grad *)calloc((size_t)nthreads, sizeof(struct sjev_grad));
    if (!p->grads) {
        if (err) snprintf(err, err_len, "oom pool grads");
        free(p);
        return NULL;
    }
    for (int i = 0; i < nthreads; i++) {
        if (grad_alloc(m, &p->grads[i], err, err_len) != 0) {
            for (int k = 0; k < i; k++) grad_free(m, &p->grads[k]);
            free(p->grads);
            free(p);
            return NULL;
        }
...
void sjev_pool_free(const struct sjev_model *m, struct sjev_thread_pool *p) {
    if (!p) return;
    for (int i = 0; i < p->nthreads; i++) grad_free(m, &p->grads[i]);
    free(p->grads);
    free(p);
}
...
int sjev_pool_run_batch(struct sjev_thread_pool *p, const struct sjev_model *m,
    ...
    if (!p || !m || !ds || !indices || !grad_out || !loss_sum) {
        if (err) snprintf(err, err_len, "invalid batch args");
        return -1;
    }
    int T = p->nthreads;
    size_t n = (end > start) ? (end - start) : 0;
    pthread_t *th = (pthread_t *)malloc((size_t)T * sizeof(pthread_t));
    struct sjev_worker_arg *args = (struct sjev_worker_arg *)malloc((size_t)T * sizeof(struct sjev_worker_arg));
    if (!th || !args) {
        free(th); free(args);
        if (err) snprintf(err, err_len, "oom batch threads");
        return -1;
    }
    /* Contiguous split, first workers take the remainder. Deterministic. */
    size_t base = (T > 0) ? n / (size_t)T : 0;
    size_t rem = (T > 0) ? n % (size_t)T : 0;
    size_t cur = start;
    for (int i = 0; i < T; i++) {
        ...
        args[i].start = cur;
        args[i].end = cur + len;
        args[i].grad = &p->grads[i];
        args[i].loss_sum = 0.0;
        args[i].rc = 0;
        args[i].err[0] = '\0';
        cur += len;
        if (pthread_create(&th[i], NULL, sjev_worker_main, &args[i]) != 0) {
            /* Best effort: join the ones already running before failing. */
            for (int k = 0; k < i; k++) pthread_join(th[k], NULL);
            free(th); free(args);
            if (err) snprintf(err, err_len, "pthread_create failed");
            return -1;
        }
    }
    ...
    int bad = -1;
    for (int i = 0; i < T; i++) {
        if (args[i].rc != 0 && bad < 0) bad = i;
    }
    if (bad >= 0) {
        if (err) snprintf(err, err_len, "worker %d failed: %s", bad,
                           args[bad].err[0] ? args[bad].err : "forward/backward error");
        free(th); free(args);
        return -1;
    }
    /* Deterministic reduction in stable worker-index order. No atomics. */
    grad_zero(m, grad_out);
    double total = 0.0;
    for (int i = 0; i < T; i++) {
        grad_add(m, grad_out, &p->grads[i]);
        total += args[i].loss_sum;
    }
    *loss_sum = total;
    free(th); free(args);
    return 0;
...
int train_loop(const struct sjev_model *model_init, const struct dataset *train, const struct dataset *valid,
    ...
    struct sjev_thread_pool *pool = sjev_pool_create(m, nthreads, err, err_len);
    if(!pool){ grad_free(m,&grad); return -1; }
    ...
    if(!best){ grad_free(m,&grad); sjev_pool_free(m,pool); if(err) snprintf(err,err_len,"oom best"); return -1; }
    ...
    if(!best->embedding||!best->position||!best->ln_c_w||!best->ln_c_b||!best->ln_o_w||!best->ln_o_b||!best->Wq||!best->Wk||!best->Wv){
        grad_free(m,&grad); sjev_pool_free(m,pool); sjev_model_free(best); if(err) snprintf(err,err_len,"oom best alloc"); return -1;
    }
    ...
    if(!indices){ grad_free(m,&grad); sjev_pool_free(m,pool); sjev_model_free(best); if(err) snprintf(err,err_len,"oom indices"); return -1; }
    ...
    for(int epoch=0; epoch<epochs; epoch++){
        ...
        size_t train_cnt=0;
        // iterate batches: workers run forward/backward on disjoint examples
        // (weights read-only); clip/scale/Adam run once after the join.
        for(size_t start=0; start<train_n; start+=batch_size){
            ...
            double batch_loss=0;
            if(sjev_pool_run_batch(pool, m, train, indices, start, end, &grad, &batch_loss, err, err_len)!=0){
                free(indices); grad_free(m,&grad); sjev_pool_free(m,pool); sjev_model_free(best); return -1;
            }
        ...
        if(eval_dataset(m, valid, &val_nll, &val_top1, &val_top3, err, err_len)!=0){ free(indices); grad_free(m,&grad); sjev_pool_free(m,pool); sjev_model_free(best); return -1; }
