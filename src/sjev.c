/*
 * sjev.c — SJEV command line: predict, train, eval, grad-check.
...
static void print_usage(const char *prog) {
    fprintf(stderr,
        ...
        "\n"
        "Train options (defaults match jevlike.train):\n"
        ...
        "  --seed INT             default 7\n"
        "  --threads INT          default 1 (CPU workers per batch, 1..1024)\n"
...
static int cmd_train(int argc, char **argv) {
    ...
    int epochs=8, batch_size=64, nthreads=1;
    ...
    for(int i=0;i<argc;i++){
        if(strcmp(argv[i],"--help")==0||strcmp(argv[i],"-h")==0){ print_usage(argv[0]); return 0; }
            ...
            if(i+1>=argc){fprintf(stderr,"--threads requires arg\n");return 1;}
            char *e=NULL; long v=strtol(argv[++i],&e,10);
            if(e==argv[i]||*e!='\0'||v<1||v>1024){fprintf(stderr,"--threads needs integer 1..1024\n");return 1;}
            nthreads=(int)v;
        }
    ...
    int rc = train_loop(m, &train, &valid, epochs, batch_size, nthreads, &adam, output_path, err, sizeof(err));
...
static int cmd_grad_check(int argc, char **argv) {
    const char *model_path=NULL,*data_path=NULL;
    int nthreads=1;
    for(int i=0;i<argc;i++){
    ...
    if(!model_path||!data_path){ fprintf(stderr,"grad-check MODEL.sjev DATA.jsonl [--threads N]\n"); return 1; }
    char err[1024]={0};
    struct sjev_model *m=NULL;
    if(sjev_model_load(model_path,&m,err,sizeof(err))!=0){ fprintf(stderr,"load %s: %s\n",model_path,err); return 1; }
    struct dataset ds;
    ...
    struct sjev_thread_pool *pool=sjev_pool_create(m,nthreads,err,sizeof(err));
    if(!pool){ fprintf(stderr,"pool %s\n",err); grad_free(m,&grad); dataset_free(&ds); sjev_model_free(m); return 1; }
    size_t *indices=(size_t*)malloc(ds.n*sizeof(size_t));
    if(!indices && ds.n){ fprintf(stderr,"oom indices\n"); sjev_pool_free(m,pool); grad_free(m,&grad); dataset_free(&ds); sjev_model_free(m); return 1; }
    for(size_t i=0;i<ds.n;i++) indices[i]=i;
    double loss_sum=0;
    if(sjev_pool_run_batch(pool,m,&ds,indices,0,ds.n,&grad,&loss_sum,err,sizeof(err))!=0){
        fprintf(stderr,"batch %s\n",err); free(indices); sjev_pool_free(m,pool); grad_free(m,&grad); dataset_free(&ds); sjev_model_free(m); return 1;
    }
    free(indices);
    sjev_pool_free(m,pool);
    /* Per-example logits use a deterministic forward-only pass; forward is
     * pure per example, so output is identical under any thread count. */
    printf("{\"logits\": [");
    for(size_t i=0;i<ds.n;i++){
        struct forward_cache c;
